$(function () {
  ...
  $("#p2-toggleBtn").click(function () {
    // 이미지 하나를 img 폴더 내에 저장 후 이미지<img src>에서
  ...
  $("#p3-menuBtn").click(function () {
    // HINT: #p3-menuPanel 요소를 .slideToggle("solw")을 사용해 슬라이드 효과를 주세요.
    $("#p3-menuPanel").slideToggle("solw");
  });

  // 문제 4: 입력값 따라쓰기
  $("#p4-myInput").on("keyup", function () {
    // HINT: $(this).val()로 입력값을 가져와서 #p4-displayText에 .text()로 표시하세요.
    const inputValue = $(this).val(); // 입력된 텍스트를 가져옴
    $("#p4-displayText").text(inputValue); // 텍스트를 표시함
  });
