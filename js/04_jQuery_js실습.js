        ...
        // 문의양식 제출 기능 
        $("#btn").click(()=>{
            // 1. 각 입력 필드 의 값을 가져와서 앞 뒤 공백 제거
            const name = $("#name").val().trim();
            const email = $("#email").val().trim();
            const massgae = $("#messge").val().trim();

            // 2. 값 증 히나라도 비어있는 확인하기 
            // addClass removeclass 활용해서 4순위 class스타일 추가하고 적용 설정
        ...
        /*
        this : 이벤트(행동, 행위) 발생한 HTML 요소 가리킴
        #menu li : id가 menu인 <ul> 태그 내부에 모든 <li>태그에 클릭 이벤트 설정 
        this : 사용자가 클릭한 <li> 태그만 바라봄 
                단순히 #menu li = this 같은 것이 아니라 
                this 는 사용자가 감지한 위치를 가리키기도함.
                $("#menu li").removeClass("active");
                    menu안에 존재하는 모든 li 태그를 바라본다면
                    
                $(this).removeClass("active");
                 this 는 사용자가 클릭한 menu li 태그를 바라보는 것 
        */
