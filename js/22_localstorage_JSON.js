...
function addData(e) {
  ...
  // 기존에 저장된 데이터가 존재하는 확인 없으면 빈배열 데이터 가져오기
  let userList = JSON.parse(localStorage.getItem("userList") || "[]");
  /**
   * Date.now() = ms 초로 변환
   * new Date().toLocalString("ko-kr");
   * ->한국 현재 시간 기준 문자열로 지정
   *
   */
  ...
  // newUser 변수 이름은 소비자가 작성한 데이터를 가지고 있응 변수이름
  // userList 로컬스토리지에 유저정보를 보유하고 추후 새로운 데이터를 저장할 변수 이름

  userList.push(newUser); //기존 userList 에 새로운 유저 추가한 상태!
  // 로컬스토리지가 아닌 javaScript 내에서만 추가 하겠다 되어있는 상태

  //  javaScript 내 추가된 데이터를 로컬스토리지에 저장
  // 로컬스토리지에 저장 할 때 배열 -> 문자열 형태로 저장
  // 사람들이 개별 컴퓨터에 데이터를 저장 하거나 특정 작업을 진행하는것을
  //  브라우저가 못하도록 방지하지만 즐겨찾기와 같이 글자 데이터를 최소한 저장할 수 있는
  // 문자열 OK했기 때문에 문자열만 가능!
...
function searchData(e) {
   ...
   * userList 에서 전달 받은 목록 중에서
   *.filter() : 걸러낼 것이다.
   * data: data라는 변수 이름에   userList 에서 가져온 정보를 하나씩 담아서
   * data.name === searchValue : data 내부에 이름과 소비자가 검색한 value 와 일치하는 것만
  ...
  if (searchResult.length > 0) {
    ...
    /**
     *
     * filter 내부에 작성된 data 는 filter 에서 소비자가 검색한 데이터와
     * 일치하는 지 확인하기 위해 사용
     *
     * map 내부에 작성된 data 는 검색 완료된 유저 목록을
     * 하나씩 꺼내서 소비자에게 걸러낸 결과값을 보여주기 위해서 활용되는 변수 이름
     *
     */
